...
export const HeroSection = () => {
  ...
  // Получаем параметр focus из URL
  ...
  // Определяем ключи для заголовков в зависимости от focus
  const getTitleKey = (part: number) => {
    const suffix = focus ? `_${focus}` : '';
    return `hero.title${part}${suffix}`;
  };

  // Определяем ключ для описания в зависимости от focus
  const getDescriptionKey = () => {
    const suffix = focus ? `_${focus}` : '';
    return `hero.description${suffix}`;
  };
    ...
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
      ...
      <div className="container-custom relative z-10">
        ...
        >
          ...
          >
            <Button
              onClick={() => window.scrollTo({ top: document.getElementById('calculator')?.offsetTop || 0, behavior: 'smooth' })}
              variant="hero"
