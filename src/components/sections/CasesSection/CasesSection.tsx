      ...
      <section id="cases" className="section-padding overflow-hidden" ref={ref}>
        <div className="container-custom">
          <div className="text-center py-20">
            <p className="text-muted-foreground">{t('common.loading', { defaultValue: 'Загрузка...' })}</p>
          </div>
        </div>
      </section>
    );
  ...
  if (error || cases.length === 0) {
    return (
      <section id="cases" className="section-padding overflow-hidden" ref={ref}>
        <div className="container-custom">
          <div className="text-center py-20">
            <p className="text-muted-foreground">{t('common.noCases', { defaultValue: 'Кейсы не найдены' })}</p>
          </div>
        </div>
      </section>
    );
            ...
            {cases.map((caseItem, index) => (
              <SwiperSlide key={caseItem.id} className="h-auto">
                <div className="group relative flex flex-col gap-4 h-full">
                  <Link to={`/cases/${caseItem.slug}`} className="relative overflow-hidden rounded-2xl cursor-pointer aspect-square block">
                    <img
