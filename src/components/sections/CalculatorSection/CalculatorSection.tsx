...
export const CalculatorSection = () => {
    ...
    // Сохраняем данные калькулятора в контекст когда все поля заполнены
    useEffect(() => {
        const isCalculatorComplete = formData.serviceType && formData.projectGoal && 
            formData.scale && formData.budget && formData.timeline;
    ...
    }, [formData, setCalculatorData]);

    const calculateResult = () => {
        const serviceData: Record<string, { base: number, weeks: [number, number] }> = {
            site: { base: 400, weeks: [1, 2] },
            app: { base: 2000, weeks: [3, 5] },
            bot: { base: 200, weeks: [1, 2] },
            automation: { base: 1600, weeks: [2, 3] },
            notSure: { base: 1000, weeks: [2, 3] },
        };

        // Если выбран сайт (web) и простой масштаб (landing), снижаем базовую цену до 130€
        if (formData.serviceType === 'site' && formData.scale === 'simple') {
            serviceData.site.base = 130;
        }
        ...
        let minWeeks = Math.max(1, Math.round((service.weeks[0] + scale.weekOffset) * timeline.timeMult));
        let maxWeeks = Math.max(minWeeks + 1, Math.round((service.weeks[1] + scale.weekOffset) * timeline.timeMult));

        // Если выбран Web + Simple (landing), устанавливаем время "до недели"
        if (formData.serviceType === 'site' && formData.scale === 'simple') {
            minWeeks = 0;
            maxWeeks = 1;
        }
                    ...
                    <div className="space-y-6 p-1">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            {Object.entries(t('cta.steps.step2.options', { returnObjects: true })).map(([key, value]) => {
                                const Icon = stepIcons[key] || Rocket;
                                return (
                                    <Button
                                        key={key}
                                        type="button"
    ...
    return (
        <section id="calculator" className="section-padding relative overflow-hidden" ref={ref}>
            ...
            <div className="container-custom relative z-10">
                ...
                <motion.div
                    ...
                    className="bg-card/40 backdrop-blur-md rounded-[2.5rem] p-8 md:p-16 border border-border relative min-h-[600px] flex flex-col shadow-2xl shadow-primary/5 max-w-4xl mx-auto"
                >
